...
apply_tsne_kcc <- function(in_path, k, out_path) {
  ...
  dr <- tsne(t(uniqColors))

  # Run clustering 
  TSN <- uniqColors %>% mutate(Cluster = kcca(dr$data, k)@cluster)
  KCentroid <- left_join(Img_DF, TSN, by = c("Red", "Green", "Blue")) %>% mutate(Cluster = as.factor(Cluster))
  ...
  # Write file
  end_string <- strsplit(in_path, "/") %>% unlist() %>% tail(1) %>% gsub(pattern = ".png", replacement = "_tSNE_KCC.txt", fixed = T)
