...
apply_svd_kcc <- function(in_path, k, out_path) {
  ...
  dr <- svd(Img_DF[,c("Red", "Green", "Blue")])
  ...
  # Write file
  end_string <- strsplit(in_path, "/") %>% unlist() %>% tail(1) %>% gsub(pattern = ".png", replacement = "_SVD_KCC.txt", fixed = T)
