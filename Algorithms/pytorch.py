...
parser.add_argument('--output_image', help='name for output image')
...
for batch_idx in range(args.maxIter):
    ...
    if args.visualize:
        ...
        plt.imshow(im_target_rgb, cmap = 'gray', interpolation = 'bicubic')
        plt.xticks([]), plt.yticks([])  # to hide tick values on X and Y axis
        plt.show()
        im_target_arr = np.array(im_target_rgb)
        #print(png_arr.shape)
        #np.save('jpg_out52', im_target_arr)
        #cv2.waitKey(10)
...
if not args.visualize:
    ...
    im_target_rgb = np.array([label_colours[ c % args.nChannel ] for c in im_target])
    im_target_rgb = im_target_rgb.reshape( im.shape ).astype( np.uint8 )
cv2.imwrite(args.output_image, im_target_rgb )
